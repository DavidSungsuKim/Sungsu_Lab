...
#ifdef _DBG
	printk(KERN_INFO "adxl345_get_triple()\n");
#endif
...
#ifdef _DBG
	printk(KERN_INFO "adxl345_position_read(), buf=%s\n", buf);
#endif
			      ...
			      const struct adxl345_bus_ops *bops)
{
...
#ifdef _DBG
	printk(KERN_INFO "adxl345_probe()-begin\n");
#endif
	...
	revid = 0xE5; // to avoid error return..

	printk(KERN_INFO "adxl345_probe(), name=%s continue probing.. L%d\n", input_dev->name, __LINE__);
#endif
...
#ifdef _DBG
	// return struct adxl345 here without writing any values to the h/w.
	printk(KERN_INFO "adxl345_probe()-done by pretending to be okay. L%d\n", __LINE__);
	return ac;
#endif
 ...
 */
 err_out:
#ifdef _DBG
	printk(KERN_INFO "adxl345_probe(), error L%d\n", __LINE__);
#endif
	return ERR_PTR(err);
}
				  ...
				  void *buf)
{
	...
	*/
...
#ifdef _DBG
	printk(KERN_INFO "adxl345_spi_read_block()\n");

	{
		...
		status = spi_write_then_read(spi, tx_dbg, count_dbg, rx_dbg, count_dbg);
		
		rx_dbg[32] = 0;
		for( i = 0; i < count_dbg; i++) {
			rx_dbg[i] += offset_ascii;
		}
