...
static int mcp251x_spi_trans(struct spi_device *spi, int len)
{
	...
	ret = spi_sync(spi, &m);	// sungsu : Here spi_sync( ) is used!
	if (ret)
...
#ifdef _DBG 
	if ( ret )
		printk(KERN_INFO "clk_prepare_enable, ret=%d\n", ret);
	ret = 0;
#endif
	if (ret)
		goto out_free;
...
#ifndef _DBG
	// Sungsu : ignore for now for testing...
